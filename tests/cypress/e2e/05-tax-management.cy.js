describe('Tax Management', () => {
  ...
  it('should create a new tax rate successfully', () => {
    ...
    cy.get('.ant-drawer.ant-drawer-open .ant-drawer-content').within(() => {
      // Fill tax name
      cy.get('input#taxName').type(taxData.name, { delay: 50 });
      cy.wait(300);
      
      // Fill tax value
      cy.get('input#taxValue').type(taxData.value, { delay: 50 });
      cy.wait(300);
    ...
    cy.get('.ant-table-tbody tr:not(.ant-table-measure-row)').then($rows => {
      if ($rows.length > 0) {
        ...
        cy.get('.ant-drawer.ant-drawer-open .ant-drawer-content', { timeout: 10000 }).should('be.visible').within(() => {
          // Modify tax value
          cy.get('input#taxValue').clear().type('20', { delay: 50 });
          cy.wait(500);
  ...
  it('should delete a tax rate', () => {
    ...
    cy.get('.ant-table-tbody tr:not(.ant-table-measure-row)').then($rows => {
      if ($rows.length > 0) {
        ...
        // Check if confirmation modal appears, if not deletion happened directly
        ...
        // Verify tax was removed
        cy.get('.ant-table-tbody tr:not(.ant-table-measure-row)').should('have.length.lessThan', initialCount);
      }
    });
  });
});
